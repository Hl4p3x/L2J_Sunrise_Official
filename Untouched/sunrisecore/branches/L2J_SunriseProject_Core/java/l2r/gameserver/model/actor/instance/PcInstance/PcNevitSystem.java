 ...
 * @author vGodFather
 */
public class PcNevitSystem extends PcExtention implements IUniqueId
{
	// Settings
	...
	public PcNevitSystem(L2PcInstance player)
	{
		super(player);
	...
	private void onPlayerLogin(OnPlayerLogin event)
	{
		if (HUNTING_BONUS_ENGINE)
		{
			...
			if ((getPlayer().getLastAccess() < (cal.getTimeInMillis() / 1000L)) && (System.currentTimeMillis() > cal.getTimeInMillis()))
			{
				getPlayer().getNevitSystem().setAdventTime(0); // Refuel-reset hunting bonus time
			}
			...
			startNevitEffect(getPlayer().getVariables().getInt("nevit_b", 0));
